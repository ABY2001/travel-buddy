...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['trip_id']) && isset($_GET['type'])) {
    ...
    try {
        ...
        if ($trip_type !== 'solo') {
            ...
            $pdo->rollBack();
            setNotification("Invalid trip type.", "error");
            header("Location: ../public/pages/dashboard.php");
            exit;
        }
        ...
        if (!$trip) {
            ...
            $pdo->rollBack();
            setNotification("Invalid trip.", "error");
            header("Location: ../public/pages/dashboard.php");
            exit;
        }
        ...
        if ($trip['gender_preference'] !== 'any' && $trip['gender_preference'] !== $user_gender) {
            ...
            $pdo->rollBack();
            setNotification("Your gender does not match the trip's preference.", "error");
            header("Location: ../public/pages/dashboard.php");
            exit;
        }
        ...
        if ($check_membership_stmt->fetch()) {
            ...
            $pdo->rollBack();
            setNotification("You have already requested to join this trip.", "error");
            header("Location: ../public/pages/dashboard.php");
            exit;
        }
        ...
        setNotification("Join request sent successfully!", "success");
        header("Location: ../public/pages/dashboard.php?section=joinable-trips");
        exit;
        ...
        setNotification("Failed to join the trip.", "error");
        header("Location: ../public/pages/dashboard.php?section=joinable-trips");
        exit;
    }
}
